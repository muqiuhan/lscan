...
def dump_header(header):
    ...
    for file_type in FILE_TYPE:
        file_types += (
            parse_flg_str(FILE_TYPE[file_type], header.file_types, file_type) + " "
        )
    ...
    # if the current node has modules
        ...
        for match in matchs:
            ...
            for module in node.modules:
                ...
                bufcrc16 = crc16(
                    buf[match.start() + 32 : match.start() + 32 + module.crc_len],
                    module.crc_len,
                )
    ...
    for sigf in sigfiles:
        ...
        print(
            ...
            % (
                ...
                len(matches),
                header.num_fcns,
                "{:.2f}".format(100 * float(len(matches)) / float(header.num_fcns)),
            )
        )
        # print the result
        if debug:
...
if __name__ == "__main__":
    ...
    lscan(
        opts.sigdir if opts.sigdir else opts.sigfile,
        opts.binfile,
        opts.verbose,
        opts.dump,
    )
